...
public class RegexParser extends GraqlParser {
  ...
  private static String regex(String regex, String... tags) {
    ...
    regex = regex.replace("<insert_has>", "(<wso>,<ws>has<ws>(?<INSERTHASIDENT><identifier><ws><wso><property_identifier>))");
    regex = regex.replace("<insert_attribute>", "(<wso>;<ws><wso>(?<INSERTATTIDENT><variable><ws><wso><property_identifier>))");
    registeredTags.add("INSERTHASIDENT");
    registeredTags.add("INSERTATTIDENT");
    regex = regex.replace("<property_identifier>", "(\"(<alphanumspace>)*\")");
    regex = regex.replace("<alphanumspace>","(<alpha>|<num>|-|<ws>|=|\\+|@|\\.)");
